...
create view valor_medio_por_tipo_pagamento as
	SELECT 
    metodo_pagamento,
    AVG(valor_total) AS valor_medio_compra
    FROM 
        venda
    GROUP BY 
        metodo_pagamento;
 
create view valor_medio_por_cliente AS
	SELECT 
    v.fk_cliente_fk_Aluno_fk_pessoa_id AS id_cliente,
    ...
    FROM 
        venda v
    JOIN 
        pessoa p ON v.fk_cliente_fk_Aluno_fk_pessoa_id = p.id
    GROUP BY 
        v.fk_cliente_fk_Aluno_fk_pessoa_id, p.nome, v.metodo_pagamento;
